/* ============================================================
   AB-100 Exam Prep  |  practice exam engine
...
(function (global) {
    ...
    function bank() { return global.AB1_DATA || {}; }
    ...
    Engine.prototype.load = function () {
        var saved = AB1.store.get(this.key, null);
    ...
    Engine.prototype.save = function () { AB1.store.set(this.key, this.state); };

    Engine.prototype.reset = function () {
        this.stopTimer();
        AB1.store.remove(this.key);
    ...
    Engine.prototype.scoreSection = function (sec) {
        ...
        return {
            ...
            pct: Math.round(pct),
            scaled: this.isDrill() ? null : AB1.rawToScaled(right, list.length),
    ...
    Engine.prototype.renderIntro = function () {
        ...
        var rows = "";
        var counted = {};
        for (var d = 0; d < AB1.DOMAINS.length; d++) {
            var dom = AB1.DOMAINS[d];
            var domN = 0, parts = [];
            for (var ti = 0; ti < AB1.TOPICS.length; ti++) {
                var tp = AB1.TOPICS[ti];
                if (tp.domain !== dom.id) continue;
                var tn = mix[tp.name] || 0;
                if (!tn) continue;
                domN += tn;
                counted[tp.name] = true;
                parts.push(AB1.escapeHtml(tp.name) + " " + tn);
            }
            if (!domN) continue;
            rows += "<tr><td><strong>" + AB1.escapeHtml(dom.name) + "</strong>" +
        ...
        for (var k in mix) {
            ...
            rows += "<tr><td><strong>" + AB1.escapeHtml(k) + "</strong></td><td>" + mix[k] +
                " questions</td><td>" + Math.round(mix[k] / totalQ * 100) + "%</td><td>&mdash;</td></tr>";
        }

        var pass = AB1.passMark();
        var needed = AB1.correctNeeded(totalQ);
    ...
    Engine.prototype.renderSectionIntro = function () {
        ...
        wrap.innerHTML =
            ...
            '<div class="badge accent">' + (this.sections.length > 1
                ...
                : "AB-100 \u00b7 Agentic AI Business Solutions Architect") + "</div>" +
    ...
    Engine.prototype.renderExam = function () {
        ...
        bar.innerHTML =
            ...
            '<span class="timer" id="clock">' + AB1.fmtClock(this.state.timeLeft[sec.id]) + "</span>" +
    ...
    Engine.prototype.qCard = function (q, sec, review) {
        ...
        head.innerHTML = '<span class="q-num">' + q.n + "</span>" +
            (q.topic ? '<span class="badge">' + AB1.escapeHtml(q.topic) + "</span>" : "") +
            (review && q.objective ? '<span class="badge accent">Obj ' + AB1.escapeHtml(q.objective) + "</span>" : "");
        if (!review) {
    ...
    Engine.prototype.renderResults = function () {
        ...
        var scaled = isDrill ? null : AB1.rawToScaled(right, total);
        var pass = AB1.passMark();
        ...
        if (isDrill) {
            ...
            var v = AB1.verdict(scaled);
            var gap = scaled - pass;
            var shortBy = Math.max(1, AB1.correctNeeded(total) - right);
            var deltaTxt = gap > 0
            ...
            wrap.appendChild(el("div", "score-hero",
                '<div class="composite-label">Estimated scaled score \u00b7 AB-100</div>' +
        ...
        for (var d = 0; d < domains.length; d++) {
            if (agg[domains[d].name]) {
                ...
                var parent = null;
                for (var pd = 0; pd < AB1.DOMAINS.length; pd++) {
                    if (AB1.DOMAINS[pd].id === domains[d].domain) parent = AB1.DOMAINS[pd];
                }
                ordered.push({
                    name: domains[d].name,
        ...
        if (ordered.length > 1) {
            ...
            for (var o = 0; o < ordered.length; o++) {
                ...
                tiles.appendChild(el("div", "score-tile",
                    '<div class="sname">' + AB1.escapeHtml(it.name) + "</div>" +
        ...
        for (var p = 0; p < results.length; p++) {
            ...
            paceRows += "<tr><td>" + AB1.escapeHtml(results[p].sec.name) + "</td><td>" + sp + " of " +
        ...
        for (var w = 0; w < weak.length; w++) {
            ...
            weakHtml += '<div class="meter-row"><span class="mlabel">Objective ' + AB1.escapeHtml(weak[w].name) + "</span>" +
        ...
        if (!weakHtml) {
            ...
            for (var y = 0; y < tw.length; y++) {
                ...
                weakHtml += '<div class="meter-row"><span class="mlabel">' + AB1.escapeHtml(tw[y].name) + "</span>" +
        ...
        this.root.appendChild(wrap);
        AB1.autoFillMeters(this.root);

        /* ---- remember the result ---- */
        if (!isDrill) {
            var hist = AB1.store.get("history", {});
            var domainSnapshot = {};
        ...
        if (this.sections.length > 1) {
            for (var i = 0; i < this.sections.length; i++) {
                tabs += '<button class="btn sm ' + (i === idx ? "" : "secondary") + '" data-sec="' + i + '">' +
                    AB1.escapeHtml(this.sections[i].name) + "</button>";
            }
        }
        head.innerHTML =
        ...
        mountDrill: function (opts) {
            ...
            if (!set) {
                opts.el.innerHTML = '<div class="card"><h3>Drill not found</h3><p>No drill set matches &quot;' +
                    AB1.escapeHtml(opts.drillId) + '&quot;.</p></div>';
                return null;
            }
            var sectionObj = {
                id: "drill_" + set.id,
