/* ============================================================
   AI-901 Exam Prep  |  practice exam engine
...
(function (global) {
    ...
    function bank() { return global.AI9_DATA || {}; }
    ...
    Engine.prototype.load = function () {
        var saved = AI9.store.get(this.key, null);
    ...
    Engine.prototype.save = function () { AI9.store.set(this.key, this.state); };

    Engine.prototype.reset = function () {
        this.stopTimer();
        AI9.store.remove(this.key);
    ...
    Engine.prototype.scoreSection = function (sec) {
        ...
        return {
            ...
            pct: Math.round(pct),
            scaled: this.isDrill() ? null : AI9.rawToScaled(right, list.length),
    ...
    Engine.prototype.renderIntro = function () {
        ...
        for (var i = 0; i < this.sections.length; i++) {
            ...
            for (var j = 0; j < list.length; j++) {
                var t = list[j].q.topic || list[j].q.tag || "Untagged";
                mix[t] = (mix[t] || 0) + 1;
            }
        }

        var domains = AI9.TOPICS;
        var rows = "";
        ...
        for (var k in mix) {
            ...
            if (!known) {
                rows += "<tr><td><strong>" + AI9.escapeHtml(k) + "</strong></td><td>" + mix[k] +
                    " questions</td><td>" + Math.round(mix[k] / totalQ * 100) + "%</td><td>&mdash;</td></tr>";
            }
        }

        var pass = AI9.passMark();
        var needed = AI9.correctNeeded(totalQ);
    ...
    Engine.prototype.renderSectionIntro = function () {
        ...
        wrap.innerHTML =
            ...
            '<div class="badge accent">' + (this.sections.length > 1
                ...
                : "AI-901 \u00b7 Azure AI Fundamentals") + "</div>" +
    ...
    Engine.prototype.renderExam = function () {
        ...
        bar.innerHTML =
            ...
            '<span class="timer" id="clock">' + AI9.fmtClock(this.state.timeLeft[sec.id]) + "</span>" +
    ...
    Engine.prototype.qCard = function (q, sec, review) {
        ...
        head.innerHTML = '<span class="q-num">' + q.n + "</span>" +
            (q.topic ? '<span class="badge">' + AI9.escapeHtml(q.topic) + "</span>" : "") +
            (review && q.objective ? '<span class="badge accent">Obj ' + AI9.escapeHtml(q.objective) + "</span>" : "");
        if (!review) {
    ...
    Engine.prototype.renderResults = function () {
        ...
        var scaled = isDrill ? null : AI9.rawToScaled(right, total);
        var pass = AI9.passMark();
        ...
        if (isDrill) {
            ...
            var v = AI9.verdict(scaled);
            var gap = scaled - pass;
            var shortBy = Math.max(1, AI9.correctNeeded(total) - right);
            var deltaTxt = gap > 0
            ...
            wrap.appendChild(el("div", "score-hero",
                '<div class="composite-label">Estimated scaled score \u00b7 AI-901</div>' +
        ...
        if (ordered.length > 1) {
            ...
            for (var o = 0; o < ordered.length; o++) {
                ...
                tiles.appendChild(el("div", "score-tile",
                    '<div class="sname">' + AI9.escapeHtml(it.name) + "</div>" +
        ...
        for (var p = 0; p < results.length; p++) {
            ...
            paceRows += "<tr><td>" + AI9.escapeHtml(results[p].sec.name) + "</td><td>" + sp + " of " +
        ...
        for (var w = 0; w < weak.length; w++) {
            ...
            weakHtml += '<div class="meter-row"><span class="mlabel">Objective ' + AI9.escapeHtml(weak[w].name) + "</span>" +
        ...
        if (!weakHtml) {
            ...
            for (var y = 0; y < tw.length; y++) {
                ...
                weakHtml += '<div class="meter-row"><span class="mlabel">' + AI9.escapeHtml(tw[y].name) + "</span>" +
        ...
        wrap.appendChild(el("div", "grid grid-2",
            ...
            '<p class="muted" style="margin-bottom:0;font-size:.88rem">The real exam gives you about 45 minutes for ' +
            "40 to 60 questions. If you are burning the whole clock here, pacing is a separate problem from knowledge and " +
        ...
        this.root.appendChild(wrap);
        AI9.autoFillMeters(this.root);

        /* ---- remember the result ---- */
        if (!isDrill) {
            var hist = AI9.store.get("history", {});
            var domainSnapshot = {};
        ...
        if (this.sections.length > 1) {
            for (var i = 0; i < this.sections.length; i++) {
                tabs += '<button class="btn sm ' + (i === idx ? "" : "secondary") + '" data-sec="' + i + '">' +
                    AI9.escapeHtml(this.sections[i].name) + "</button>";
            }
        }
        head.innerHTML =
        ...
        mountDrill: function (opts) {
            ...
            if (!set) {
                opts.el.innerHTML = '<div class="card"><h3>Drill not found</h3><p>No drill set matches &quot;' +
                    AI9.escapeHtml(opts.drillId) + '&quot;.</p></div>';
                return null;
            }
            var sectionObj = {
                id: "drill_" + set.id,
